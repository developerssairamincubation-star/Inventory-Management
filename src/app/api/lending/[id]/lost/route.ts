...
export async function POST(
  ...
  try {
    ...
    const [lendingItem] = await db.select({ quantity: lending_item.quantity, lost_quantity: lending_item.lost_quantity, item_type: lending_item.item_type }).from(lending_item).where(and(eq(lending_item.lend_order_id, id), eq(lending_item.product_id, product_id)))
    if (!lendingItem) {
      return NextResponse.json({ error: "Lending item not found" }, { status: 404 });
    }
    if (lendingItem.item_type !== 'RETURNABLE') {
      return NextResponse.json({ error: "Consumable items can't be marked lost" }, { status: 400 });
    }
