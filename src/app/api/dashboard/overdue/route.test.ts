...
beforeAll(async () => {
  ...
  const [borrower] = await db.insert(staffs).values({ name: "Dash Overdue Borrower", department_id: DEPT_ID }).returning();
...
describe("GET /api/dashboard/overdue", () => {
  ...
  it("only flags PENDING orders past their due_date, joined with borrower/product names", async () => {
    ...
    const [overdue] = await db.insert(lending_order).values({ borrower_type: "STAFF", borrower_staff_id: BORROWER_ID, issued_by_user_id: OWNER_ID, status: "PENDING", due_date: pastDue }).returning();
    ...
    await db.insert(lending_order).values({ borrower_type: "STAFF", borrower_staff_id: BORROWER_ID, issued_by_user_id: OWNER_ID, status: "PENDING", due_date: future });

    // Past due but already RETURNED: should not appear.
    await db.insert(lending_order).values({ borrower_type: "STAFF", borrower_staff_id: BORROWER_ID, issued_by_user_id: OWNER_ID, status: "RETURNED", due_date: pastDue });
